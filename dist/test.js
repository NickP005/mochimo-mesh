...
const wallet_manager_1 = require("./wallet_manager");
async function testMochimoRosettaClient() {
    const client = new wallet_manager_1.MochimoRosettaClient();
    const wots = new wallet_manager_1.WOTS();
    wots.init();
    try {
        ...
        const deriveResponse = await client.constructionDerive(Buffer.from(publicKey).toString('hex'));
        ...
        const parseResponse = await client.constructionParse(payloadsResponse.unsigned_transaction, false);
        console.log('Parse Response:', parseResponse);
        // Test construction combine
        console.log('\n9. Testing Construction Combine...');
        const signature = wots.generateSignatureFrom(testSeed, Buffer.from(payloadsResponse.unsigned_transaction));
        const signatures = [{
        ...
        const combineResponse = await client.constructionCombine(payloadsResponse.unsigned_transaction, signatures);
        console.log('Combine Response:', combineResponse);
        // Test construction hash
        console.log('\n10. Testing Construction Hash...');
        const hashResponse = await client.constructionHash(combineResponse.signed_transaction);
